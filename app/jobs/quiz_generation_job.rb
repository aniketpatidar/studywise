class QuizGenerationJob < ApplicationJob
  queue_as :default
  ...
  retry_on StandardError, wait: 5.seconds, attempts: 3 do |job, error|
    ...
    Study::GenerationTracker.fail_quiz_generation(material_id: material.id, error_message: error.message)
    broadcast_material_state(material)
  end

  def perform(material_id, idempotency_key)
    material = Material.find(material_id)
    Study::QuizGenerationService.new(material: material, idempotency_key: idempotency_key).call
    Study::GenerationTracker.finish_quiz_generation(material_id: material.id)
    Study::GenerationTracker.clear_quiz_error!(material.id)
  ...
  def self.broadcast_material_state(material)
    ...
    Turbo::StreamsChannel.broadcast_replace_to(
      material,
      target: ActionView::RecordIdentifier.dom_id(material, :quizzes_panel),
      partial: "materials/quizzes_panel",
      locals: { material: material, quizzes: material.quizzes.recent }
    )
  end
end
